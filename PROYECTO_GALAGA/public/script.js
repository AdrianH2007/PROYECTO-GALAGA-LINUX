...
function spawnEnemy() {
    if (isGameOver) return; // Si terminó el juego, no genera más enemigos
...
function update() {
    if (isGameOver) {
        ...
        // ENVIAR PUNTUACIÓN A MONGODB AL MORIR
        if (!scoreSent) {
            scoreSent = true; 
            fetch('/api/highscore', {
                method: 'POST',
            ...
            .then(() => console.log("Puntaje guardado en MongoDB"))
            .catch(err => console.error("Error al guardar puntuación:", err));
        }
        return; // Detiene la ejecución de este frame permanentemente
    }
...
async function loadHighScore() {
    try {
        ...
        document.getElementById("scoreBoard").innerHTML = `Puntuación: <span id="score">0</span> | Récord: <span>${highscore}</span>`;
    } catch (err) {
        console.error("No se pudo cargar el récord", err);
    }
}
loadHighScore();
